 ...
 */
public class NotificationPortlet extends GenericPortlet
{
    ...
    throws UnavailableException, PortletSecurityException, PortletException, IOException
    {
        ...
        String mark_read = actionRequest.getParameter("MARK_READ");
        String cancel = actionRequest.getParameter("CANCEL");

    	psession.setAttribute("COMMAND", command);
    	psession.setAttribute("CANCEL", cancel);
    	psession.setAttribute("MARK_READ", mark_read);
        ...
        //TODO: alert box
        if ("Purge Deleted".equalsIgnoreCase(command) || "Delete this".equalsIgnoreCase(command)) {
            String[] delete_msg_ids = (String[]) actionRequest.getParameterValues("delete_msg_id");
            System.out.println("!!!!! Delete Act!!!!!:" + delete_msg_ids[0]);
            if (delete_msg_ids != null) {
                deleteMessagesForUser(userId, getMessageIds(delete_msg_ids));
            }
        }
        else if (null != mark_read)
            {
                //String[] mark_msg_ids = (String[]) actionRequest.getParameterValues("mark_msg_id");
        	String mark_msg_id = (String) actionRequest.getParameter("DISPLAY_MSG_ID"); 
        	//System.out.println("!!!!!command mark_msg_id!!!!!:" + mark_msg_id);
        	//System.out.println("test:" + psession.getAttribute("display_msg_id"));
        	String[] mark_msg_ids = new String[]{mark_msg_id};
            if (mark_msg_ids != null) {
                //System.out.println("!!!!!mark_msg_ids!!!!!:" + mark_msg_ids[0]);
                markReadForUser(userId, getMessageIds(mark_msg_ids));
            }
        }	
        else if ("DISPLAY".equals(command) || "EMAIL".equals(command))
            {
                //mark/update opened
                ...
                String message_from = (String) actionRequest.getParameter("message_from");
                String message_subject = (String) actionRequest.getParameter("message_subject");
                String expiry = (String) actionRequest.getParameter("expiry");
                String release = (String) actionRequest.getParameter("release");
                String read = (String) actionRequest.getParameter("read");
                
                
                if ( "EMAIL".equals(command)) {
                	String email_text = (String)psession.getAttribute("email_text");
                	email_text = email_text.substring(11);
                	StringBuffer sb_email_text = new StringBuffer();
                	sb_email_text.append("Forwarded from MyPortico: UofG Notifications\n").append(email_text);

                	actionResponse.setRenderParameter("from", mail);
	                actionResponse.setRenderParameter("to", mail);
	                actionResponse.setRenderParameter("subject", message_subject);
	                actionResponse.setRenderParameter("server", "outbound.mail.uoguelph.ca");
	                actionResponse.setRenderParameter("message", sb_email_text.toString());
	                actionResponse.setRenderParameter("cc", "ljairath@uoguelph.ca");
	                actionResponse.setRenderParameter("X-Forwarded-By", "MyPortico: UofG Notifications");
	                //System.out.println("!!!!!sbemail!!!!!:" + sb_email_text);
                }
                psession.setAttribute("message_from", message_from);
                psession.setAttribute("message_subject", message_subject);
                psession.setAttribute("expiry", expiry);
                psession.setAttribute("read", read);
                psession.setAttribute("release", release);
    ...
    throws PortletException, IOException
    {
        ...
        String command = (String) psession.getAttribute("COMMAND");
    	String cancel = (String) psession.getAttribute("CANCEL");
    	String mark_read = (String) psession.getAttribute("MARK_READ");

        //command = "syndicate";
        //System.out.println("!!!!!commandView!!!!!:" + command);
        //System.out.println("!!!!! ServerView!!!!!:" + request.getParameter("server"));
        ...
        if ("syndicate".equalsIgnoreCase(command)) {
            ...
            */
        } else if ("DISPLAY".equals(command) || "EMAIL".equals(command) || "syndicate".equalsIgnoreCase(command)) {
            //mark update opened 
            ...
            psession.setAttribute("email_text", message_text);
            ...
            if ("DISPLAY".equals(command)) {
            	prd = portletContext.getRequestDispatcher("/WEB-INF/jsp/displayMessage.jsp");
            } else if ("EMAIL".equals(command)) {
            	prd = portletContext.getRequestDispatcher("/WEB-INF/jsp/emailMessage.jsp");
            }
            prd.include(request, response);
            
        } else if (null != mark_read || "Purge Deleted".equals(command)) { 
            ...
            PortletRequestDispatcher prd = null;
            prd = portletContext.getRequestDispatcher("/WEB-INF/jsp/ufg_notifications.jsp");
            prd.include(request, response);
        } else if ("Release".equals(command)) {
            ...
            PortletRequestDispatcher prd = portletContext.getRequestDispatcher("/WEB-INF/jsp/ufg_notifications.jsp");
            prd.include(request, response);
