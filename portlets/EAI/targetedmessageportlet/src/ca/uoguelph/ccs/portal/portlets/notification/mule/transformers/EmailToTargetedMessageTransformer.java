...
public class EmailToTargetedMessageTransformer extends AbstractEventAwareTransformer
{
	...
	public Object transform(Object src, UMOEventContext context) throws TransformerException
	{
		...
		try {
			...
			//message payload is plain/text OR plain/html
			StringBuffer partSb = new StringBuffer();
			String payload = getTextHTMLPart(part, partSb);
			//String payloadLink = setPayLoadLinks(payload); 
			//targetedMessageMap.put("message", payloadLink);
	...
	private static String getTextHTMLPart(Part p, StringBuffer partSb) throws Exception {
		...
		if (p.isMimeType("multipart/*")) {
			...
			StringBuffer htmlPartSb = new StringBuffer("text/html\n");
			if(partSb != null || partSb.length() != 0) {
				partSb = htmlPartSb;
			}
			partSb.append((String)p.getContent());
	 ...
	 * @return StringBuffer as content 
	 * @throws Exception
	 * 
	 * user ids - emails are sent as attachement and read as inputstream
	 */
	private static String getMessageContents(Part p) throws Exception{
	...
	private static String setPayLoadLinks(String payload) {
		final String ONCLICK = "onclick=\"window.open('','tmessage','resizable=yes,location=no,height=600,width=800,toolbar=yes,scrollbars=yes')\"";
		final String ANCHOR = "a";
		final String START_TAG = "<";
		final String END_TAG = ">";
		final String DQUOTE = "\"";
		final String HREF_DELIMITER = "href";
		final String HTTP_DELIMITER = "http://";
		final String TARGET = "target=\"tmessage\"";
        final String NEW_LINE = System.getProperty("line.separator");
		
		String person = "How are you doing? http://www.studentlife.uoguelph.ca/orientation/documents/InternationalStudentsSneakPeek.pdf. Things are looking good ";
		//System.out.println("person:" + payload);
		String[] http = payload.split(HTTP_DELIMITER);
		
		StringBuffer sb = new StringBuffer();
		for (int i=0; i<http.length; i++) {
			//System.out.println("http:" + http[i]);
			////System.out.println("\n" + i + "" + sb);
			if (i<(http.length-1) ){
				sb.append(" ").append(START_TAG + ANCHOR).append(" ").
				append(TARGET).append(" ").append(HREF_DELIMITER).append("=").append(DQUOTE).append(HTTP_DELIMITER) ;
			}
			if (i>0) {
				String tokens[] = http[i].split("\\s+",2);
				if (tokens[0].endsWith(".")) {
					int len = tokens[0].length();
					//System.out.println("len:" + len);
					tokens[0] = tokens[0].substring(0,len-1);
					sb.append(tokens[0]).append(DQUOTE).append(END_TAG+START_TAG+"/").append(ANCHOR+END_TAG).append(".").append(" ").append(tokens[1]);
					
				} else {
					//System.out.println("token length" + "" + tokens.length);
					sb.append(tokens[0]).append(DQUOTE).append(END_TAG+START_TAG+"/").append(ANCHOR+END_TAG).append(" ").append(tokens[1]);
		...
		////System.out.println("Payload link" + sb);
        return sb.toString();
	}
}