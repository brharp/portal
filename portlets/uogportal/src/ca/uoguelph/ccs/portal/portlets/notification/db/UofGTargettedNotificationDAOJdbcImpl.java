...
public class UofGTargettedNotificationDAOJdbcImpl implements UofGTargettedNotificationDAO
{
    ...
    private static final String displayMessages = 
        "SELECT message_id, message FROM ufg_targetted_notifications WHERE message_id IN " ; //run index query
	...
	private static final String selectUserInRoleNewNotifications = 
        ...
        " role  LIKE ?" + " OR " +
        " owner = ? ) " +
        " AND (expiry > NOW() OR expiry IS NULL ) AND message_id > ? ";
    ...
    {
        //System.out.println(displayMessages + "(" + messageIds + ")");
        return jdbcTemplate.queryForList(displayMessages + "(" + messageIds + ")");
    }
	...
	public void insertTargettedNotification(UofGTargettedNotification targettedNotification)
	{
        ...
        importance = targettedNotification.getMessageImportance();
        if (null == importance) {
        	importance = "Normal";
        }
