...
<c:catch var="exception">    
    ...
    <% 
	    //<log:info>view.jsp.</log:info>
	    settingsHandler.setSettingsBean(settingsBean);
	    String proxy = settingsHandler.PROXY;
    ...
    %>
          <table border="0" style="align: center">
           <tr>
           <td align="left">
				<p><b><i>To add/delete news content click on the edit icon above</i></b></p>
          	</td>
            </tr>        
			</table>            
 -->

	<form name="goForm" method="POST" action="<portlet:actionURL></portlet:actionURL>" target="_self">                        
                    ...
                    <tr><td>
                        <div class="portlet-font">
                            <c:if test="${feedBean.feed.image != null}">
                                ...
                                src="${proxy}${feedBean.feed.image.url}"/></a>    
                                <br/>
                            </c:if>
                            <a href="${feedBean.feed.link}" style="font-weight: bolder" 
                            target="${settingsBean.windowTarget}">${feedBean.feed.title}</a>
...
value="${feedBean.feed.publishedDate}"/></span>
                ...
                <c:forEach var="entry" items="${entries}">
                    <tr><td>
                        <div class="portlet-font">
			<img src="https://myportico.uoguelph.ca/portal/html/myportico/ccs/campusnews/images/article.png" />
                            <a href="${entry.link}" target="${settingsBean.windowTarget}">
                            ${entry.title}</a>
                            <c:if test="${entry.publishedDate != null}">
                                - <span style="font-style: italic">
                                <fmt:formatDate value="${entry.publishedDate}"/></span> 
                            </c:if>
                            <br>
