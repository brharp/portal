...
<c:catch var="exception">    
    ...
    <% 
    	String[] common_feeds = new String[] {
    		"http://feeds.feedburner.com/uoguelph/president", 
    	...
    	String[] default_feeds = new String[] {"http://www.uoguelph.ca/portal/resources/defaultcampuslife.xml"};
    	
        PortletSession psession = renderRequest.getPortletSession();
        psession.setAttribute("common_feeds", common_feeds, PortletSession.PORTLET_SCOPE);
        psession.setAttribute("student_feeds", student_feeds, PortletSession.PORTLET_SCOPE);
        psession.setAttribute("grad_student_feeds", grad_student_feeds, PortletSession.PORTLET_SCOPE);
        psession.setAttribute("faculty_feeds", faculty_feeds, PortletSession.PORTLET_SCOPE);
        psession.setAttribute("staff_feeds", staff_feeds, PortletSession.PORTLET_SCOPE);
        psession.setAttribute("default_feeds", default_feeds, PortletSession.PORTLET_SCOPE);
...
//      so we can get them from a portlet action
		...
		//System.out.println("settingsBean.feedsSize" + settingsBean.getFeedsSize());
		//System.out.println("settingsBean.feedsSize" + settingsBean.getFeeds();
		LinkedList feeds = settingsBean.getFeeds();
		
		ListIterator it = feeds.listIterator();
		LinkedList newFeeds = new LinkedList();
		while (it.hasNext()) {
			String s = (String) it.next();
			//System.out.println("substring" +  s.substring(5));
			newFeeds.add(s.substring(5));
			//System.out.println("before: " + s);
		}
		String selectedFeed = settingsBean.getSelectedFeed();
		pageContext.setAttribute("newFeeds", newFeeds);
		//System.out.println("sel feed" +  selectedFeed);
		pageContext.setAttribute("selectedFeed", selectedFeed);
...
value="${settingsBean.windowTarget}"/>"><c:out value="${entry.title}"/></a>
                            ...
                            <% // Include modules in entry rendering. Modules are extensions to     %>
                            <% // Rome feed parsing that allow for recovery of namespaced elements. %>
                            <div style="font-size: smaller">
